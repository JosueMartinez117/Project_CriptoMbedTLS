...
void Signing_Hash_ECDSA()
{
	mbedtls_printf("\r\n--- DEBUG-INFO: Signing_Hash_MSG function called  ---\r\n");
	int i;

/*Same struct than bench mark*/
/*Enable the elliptic curve over GF(p) library.*/
    ...
    /*ECDSA structure with secret or public value
     * ECDSA Signing*/
    mbedtls_printf("\r\nECDSA Signing:\r\n");
         ...
         curve_info++ )
    {
        ...
        TIME_PUBLIC( title, "sign",
                    ret = mbedtls_ecdsa_write_signature( &ecdsa, MBEDTLS_MD_SHA256, hash, curve_info->bit_size,
                                                sign, &sig_len, myrand, NULL ) );
    ...
    mbedtls_printf( "\n\r---SIGNATURE VERIFICATION---\n\r" );
   	         ...
   	         curve_info++ )
   	    {
   	        ...
   	        if( mbedtls_ecdsa_genkey( &ecdsa, curve_info->grp_id, myrand, NULL ) != 0 ||
   	            mbedtls_ecdsa_write_signature( &ecdsa, MBEDTLS_MD_SHA256, hash, curve_info->bit_size,
   	            							   sign, &sig_len, myrand, NULL ) != 0 )
   	        {
   	        	mbedtls_exit( 1 );
   	        }
   	        ecp_clear_precomputed( &ecdsa.grp );
   	        ...
   	        TIME_PUBLIC( title, "verify",
   	                    ret = mbedtls_ecdsa_read_signature( &ecdsa, hash, curve_info->bit_size,
   	                    		                            sign, sig_len ) );
   	        if(ret == 0)
   	        	mbedtls_printf("\n\r---SIGNATURE VERIFIED \n\r");
    ...
    mbedtls_printf("\r\n--- DEBUG-INFO: Signing_Hash_MSG function completed  ---\r\n");
